...
function stringifyPositions() {
    ...
    if (states.positions.length > 0) {
        text += states.positions[0].side + Math.round(states.positions[0].percent * 100) / 100.0;
        for (var i = 1; i < states.positions.length; i++) {
            var pos = states.positions[i];
            text += "," + pos.side + Math.round(pos.percent * 100) / 100.0;
        }
    }
    return text;
}
                    ...
                    if (command == "port") {
                        ...
                        for (var i = 0; i < parts.length; i++) {
                            values.positions.push({
                                side: parts[i].charAt(0),
                                percent: parseFloat(parts[i].substring(1), 10),
                            });
                        }
                    } else {
