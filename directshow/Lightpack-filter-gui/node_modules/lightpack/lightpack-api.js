...
function stringifyPositions() {
    ...
    if (states.positions.length > 0) {
        var pos = states.positions[0],
            rect = states.positions[0].rect;
        text += pos.side + signifDigits(pos.percent, 2) + ";"
            + signifDigits(rect.x, 2) + "," + signifDigits(rect.y, 2) + ","
            + signifDigits(rect.width, 2) + "," + signifDigits(rect.height, 2);
        for (var i = 1; i < states.positions.length; i++) {
            pos = states.positions[i];
            rect = states.positions[i].rect;
            text += "|" + pos.side + signifDigits(pos.percent, 2) + ";"
                + signifDigits(rect.x, 2) + "," + signifDigits(rect.y, 2) + ","
                + signifDigits(rect.width, 2) + "," + signifDigits(rect.height, 2);
        }
                    ...
                    if (command == "port") {
                        ...
                        for (var i = 0; i < parts.length; i++) {
                            var item = parts[i].split(";"),
                                rectVals = item[1].split(",");
                            values.positions.push({
                                side: item[0].charAt(0),
                                percent: parseFloat(item[0].substring(1), 10),
                                rect: {
                                    x: parseFloat(rectVals[0], 10),
                                    y: parseFloat(rectVals[1], 10),
                                    width: parseFloat(rectVals[2], 10),
                                    height: parseFloat(rectVals[3], 10),
                                }
                            });
                        }
                    } else {
