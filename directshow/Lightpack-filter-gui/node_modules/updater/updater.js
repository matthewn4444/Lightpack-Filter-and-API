  ...
  updater.prototype.download = function(cb){
    var pkg = request(this.newManifest.packages[platform]);
    var filename = path.basename(this.newManifest.packages[platform]);
  ...
  updater.prototype.downloadInstaller = function(cb){
    var installer = request(this.newManifest.installers[platform]);
    var filename = path.basename(this.newManifest.installers[platform]);
    installer.pipe(fs.createWriteStream(path.join(os.tmpdir(), filename)));
    installer.on('end', installerDownloaded);

    function installerDownloaded(){
      cb(null, path.join(os.tmpdir(), filename))
    }
    return installer;
  }
