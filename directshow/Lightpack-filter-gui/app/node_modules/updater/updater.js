  ...
  // downloadInstaller(cb) - downloads new version of the installer in temp
  ...
  var copyRecursive = function(src, dest, callback) {
        ...
        fs.mkdir(dest, function() {
          fs.readdir(src, function(err, files) {
            ...
            files.forEach(function(child) {
              copyRecursive(path.join(src, child), path.join(dest, child), function(err) {
                if (++i == files.length) {
                  callback();
                }
              });
  ...
  function download(url, cb, redirectNum) {
    redirectNum = redirectNum || 0;
    var cmd = url.indexOf("https:") != -1 ? https : http;
    cmd["get"](url, function(res) {
      if (res.statusCode > 300 && res.statusCode < 400 && res.headers.location) {
        // Redirect
        if (redirectNum < 10) {
          download(res.headers.location, cb, ++redirectNum);
        } else {
        ...
        // Most likely error or unhandled
        cb(new Error("Failed to download because of status code " + res.statusCode));
      }
    }).on("error", cb);
  }
      ...
      request.addEventListener("load", function() {
        if (callback) {
          if (this.readyState == 4 && this.status == 200) {
            ...
            callback(new Error("Unable to get from url"));
          }
        }
      });
  ...
  updater.prototype.__downloadFile = function(url, cb) {
    var filename = path.basename(url);
    var file = fs.createWriteStream(path.join(os.tmpdir(), filename));

    // Emulate passing the response object back
    var proxy = {
      data: null, response: null,
      on: function(event, cb) {
        if (this.hasOwnProperty(event)) this[event] = cb;
        return this;
      }
    };
    ...
    download(url, function(e, res) {
      ...
      if (proxy.response) proxy.response(res);
      if (proxy.data) res.on("data", proxy.data);
      res.pipe(file);
      res.on("end", function() {
  ...
  updater.prototype.download = function(cb) {
    return this.__downloadFile(this.newManifest.packages[platform], cb);
  }

  // download the package to template folder
  updater.prototype.downloadInstaller = function(cb){
    return this.__downloadFile(this.newManifest.installers[platform], cb);
  }
  ...
  var pUnpack = {
    ...
    win: function(filename, cb){
      ...
      exec('"' + path.resolve(__dirname,'tools/unzip.exe') + "\" -u -o " +
        filename + " -d " + path.join(os.tmpdir(), theName), function(err){
          if(err){
            return cb(err);
          }
          cb(null, path.join(os.tmpdir(), theName));
        });
    },
 ...
 /*     var gzip = new targz();
  ...
  var pInstall = {
    mac: function(to, cb){
      copyRecursive(this.getAppPath(), to, cb);
    },
    win: function(to, cb){
      deleteApp(appDeleted.bind(this));
      function appDeleted(err){
        copyRecursive(this.getAppPath(), to, appCopied.bind(this));
      }
