...
function startServer(port, host) {
        ...
        function rConnect(i) {
            if (i < max_attempts) {
                internalConnect(function(success){
                    ...
                    } else if (!states.isOnWhenClose) {
                        // Display the current colors last recorded
    ...
    .on("connect", function() {
        console.log("filter connected to lights");
        currentObj = filter;
        notifyConnect();
        sendSettingsFolderToFilter();
    })
...
function sendSettingsFolderToFilter() {
    if (settingsPath != path.dirname(process.execPath)) {
        filter.sendSettingsFolder(settingsPath);
    }
}
...
function saveSettings(callback) {
    ...
    data += "\n[Event Data]\n";
    for (var name in videoEventData) {
        var d = videoEventData[name];
        if (d.start && d.end && d.url) {
            data += "event" + name.capitalize() + "=" + (d.enabled ? 1 : 0) + "," + d.start + "," + d.end + "," + d.url + "\n";
        }
    }
    data += "eventDelay=" + (videoEventData.delayedTime || 0) + "\n";
...
function parseEventLine(line) {
    ...
    var url = line.substring(parts[0].length + parts[1].length + parts[2].length + 3);
    var timePattern = "\\b(1[0-2]|[0-9]):[0-5][0-9][ap]m";

    // Check for valid url
    if (!url.match("(\\b(https?|ftp|file)://)?[-A-Za-z0-9+&@#/%?=~_|!:,.;]+[-A-Za-z0-9+&@#/%=~_|]")) {
        return null;
    }

    // Check valid dates
    if ((parts[1] != "anytime" && !parts[1].match(timePattern))
            || (parts[2] != "anytime" && !parts[2].match(timePattern))) {
        return null;
    }
    return {
        enabled: parts[0] == '1',
        ...
        if (exists) {
            fs.readFile(file, function(err, contents) {
                ...
                for (var i = 0; i < lines.length; i++) {
                    ...
                    if (command == "port") {
                        values.port = parsePositiveNumber(value);
                    ...
                    } else if (command == "eventPlay") {
                        values.eventPlay = parseEventLine(value);
                    } else if (command == "eventPause") {
                        values.eventPause = parseEventLine(value);
                    } else if (command == "eventClose") {
                        values.eventClose = parseEventLine(value);
                    } else if (command == "eventDelay") {
                        values.eventDelay = parsePositiveNumber(value);
                    } else {
                        console.warn("Error: INI file has unknown command", command);
                    }
                }
...
function internalConnect(callback, tryPrismatik, opts) {
    ...
    if (currentObj == filter) {
        filter.signalReconnect(function(success){
            console.log("signalReconnect", success);
            if (success) {
                notifyConnect();
            }
            callback.call(api, success);
        ...
        if (!connectDevice()) {
            if (tryPrismatik) {
                //console.log("try to connect to prismatik")
...
function setExportedGamma(value, callback) {
    ...
    console.log("Gamma", Math.pow(10, exponent));
...
function saveVideoEventData(name, enabled, url, startTime, endTime) {
    if (videoEventData.hasOwnProperty(name)) {
        videoEventData[name].enabled = enabled;
        if (url != null) {
            videoEventData[name].url = url;
        }
        videoEventData[name].start = startTime;
        videoEventData[name].end = endTime;
        setStateChanged();
    }
}
...
function setDelayedTimeBetweenEvents(timeMs) {
    if (!isNaN(timeMs) && timeMs >= 0 && Math.floor(timeMs) == timeMs
            && videoEventData.delayedTime != timeMs) {
        videoEventData.delayedTime = timeMs;
        setStateChanged();
    }
}

function getVideoEventData(name) {
    if (!videoEventData[name] || !videoEventData[name].url
            || !videoEventData[name].start || !videoEventData[name].end) {
        return null;
    }
    return {
        enabled: videoEventData[name].enabled,
        url: videoEventData[name].url,
        start: videoEventData[name].start,
        end: videoEventData[name].end
    }
}
...
exports.setDelayedTimeBetweenEvents = setDelayedTimeBetweenEvents;
exports.getDelayedTimeBetweenEvents = function() { return videoEventData.delayedTime; }
...
exports.saveVideoEventData = saveVideoEventData;
exports.getVideoEventData = getVideoEventData;
...
exports.init = function(callback){
    ...
    readSettings(function(data){
        if (data) {
            ...
            if (data.eventPlay) {
                videoEventData.play = data.eventPlay;
            }
            if (data.eventPause) {
                videoEventData.pause = data.eventPause;
            }
            if (data.eventClose) {
                videoEventData.close = data.eventClose;
            }
            if (data.eventDelay) {
                videoEventData.delayedTime = data.eventDelay;
            }
