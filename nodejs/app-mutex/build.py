...
def main():
    ...
    res = subprocess.call("\"" + program_files + DEVENV_LOC + "\" -upgrade build/app-mutex.vcproj", shell=True, stdout=subprocess.PIPE)
    if res != 0:
        raise Exception('Failed to upgrade visual studio project')

    # Build visual studio project
    print "Building visual studio project"
    res = subprocess.call("\"" + program_files + DEVENV_LOC + "\" build/app-mutex.vcxproj -build Release", shell=True, stdout=subprocess.PIPE)
    if res != 0:
